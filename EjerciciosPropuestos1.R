...
print("d)")
mean(iris[(iris["Species"] == "versicolor")&(iris["Sepal.Length"]>6.3),"Petal.Length"])
print("e)")
...
print("Ejercicio 1")
l <- list( Species = sample(iris$Species,size=20), iris=iris, m = matrix(1:100, ncol=4))
print("a)")
lapply(l,summary)
print("b)")
...
replicate( 1, points(n,v.lanzamiento(n=n),type="p", cex=1, pch=21, bg='black'))
...
closestNumber2 <- function(n) {
  if (n-trunc(n)>=0.5) {return(trunc(n+1))}
  else {return(trunc(n))}
}
closestNumber2(1.5)
closestNumber2(1.2)
...
spambase.data[,ncol(spambase.data)] <- factor(spambase.data[,ncol(spambase.data)],labels =c("spam","non-spam"),levels=1:0)
